...
function ContentDetail({ p, moveContent, toast }: { p: Project; moveContent: (id: string, dir: 1 | -1) => void; toast: (s: string) => void }) {
  ...
  return (
    <div className="card card-pad">
      ...
      {(p.creativeKnowledge?.length || 0) > 0 && <div className="creative-knowledge"><div className="section-head"><span className="section-title">创作知识库</span><span className="t-cap">{p.creativeKnowledge!.length} 条沉淀</span></div>{p.creativeKnowledge!.slice(0, 8).map(k => <div key={k.id} className="knowledge-card"><div className="knowledge-head"><div className="t-sub">{k.title}</div><button className="knowledge-delete tap" onClick={() => { store.deleteCreativeKnowledge(p.id, k.id); toast('已删除这条知识') }}>删除</button></div><div className="t-body">{k.content}</div></div>)}</div>}
    </div>
  )
}
